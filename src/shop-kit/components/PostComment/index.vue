@@ -0,0 +1,375 @@
<template>
  <div class="sk-post-comment" data-com="SkPostComment" v-if="isShowComment">
    <!-- <div class="a"></div>
    <div class="b"></div> -->
    <div class="s-comment">
      <h4 class="s-title">
...
function getComment (id, page) {
  return window.fetch(`/api/v1/post/comments?id=${id}&page=${page || 1}&size=30`, {
    credentials: 'include'
  })
}
function getSetting (page) {
  return window.fetch(`/api/v1/post/comments/setting`, {
    credentials: 'include'
  })
}
export default {
  ...
  data () {
    ...
    return {
      is_empty: true,
      postHandle: window.location.pathname.split('/posts/')[1],
      comments: [],
      input_num: 0,
      imgId: '',
      id: '',
  ...
  methods: {
    init () {
      this.$sdk.post.get(this.postHandle, ({ res }) => {
        if (res.code === 200) {
          this.id = res.post.id || ''
    ...
    fnSubmit () {
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          let _this = this
          window.fetch(`/api/v1/post/comments/create`, {
            method: 'POST',
            headers: {
